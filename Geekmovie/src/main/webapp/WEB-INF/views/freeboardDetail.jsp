	...
	window.onload = function() {
		...
		$.fn.activeLikeButtons = function() { // - 이미 눌린 좋아요 버튼 active효과주기
			this.each(function(i, e) {
						...
						$
								.ajax({
									url : 'freeboard/like/${data.seq}?userId=${sessionScope.id}&mode='
											+ mode,
									method : "GET",
		...
		$.ajax({ //로드완료하면 좋아요 개수 세기
					...
					success : function(data) {
						document
								.querySelector(".like-button .like-dislike-button-span").innerText = data;
					},
					error : function(request, status, error) {
						console.log(request, status, error)
					}
													...
													$
															.ajax({
																...
																success : function(
																		data) {
																	likeButton
																			.querySelector(".like-dislike-button-span").innerText = data;
																	document
																			.querySelector('.movieLikes').innerText = "좋아요 : "
																			+ data;
																},
																error : function(
																		request,
